...
- Severity scale is shared across all reviewers (the three default lenses plus the optional scalability lens): P0/P1 block shipping — except that a finding the author routed to the spec's `## Deferred — follow-up required` section, under the scope ceiling, is not re-filed by the reviewers and so never enters the gate; P2+ do not. Reserve P0/P1 for genuinely load-bearing issues.
